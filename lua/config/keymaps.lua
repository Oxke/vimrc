local function map(mode, short, long, desc)
  if not desc then
    desc = long
  end

  vim.keymap.set(mode, short, long, { desc = desc, silent = true })
end

-- Moving around, tabs, windows and buffers --
----------------------------------------------

map("n", " ", "<c-w>", "easier move-around with space")
...
map("n", "<leader>tv", ":e ~/.config/nvim/lua<cr>", "open settings")
...
map("n", "<leader>gg", '!git commit -a -m "AutoCommit" && git push<cr>', "quickly commit changes")
...
-- sudo save. WARN: this requires the user to write the password in the first
